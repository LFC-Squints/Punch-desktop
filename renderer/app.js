...
function renderAccountsList(){
  ...
  state.accounts.forEach(a=>{
    ...
    row.innerHTML=`
      ...
      <button class="icon-btn" data-rename-account="${a.id}" title="Rename">✎</button>
      <button class="icon-btn" data-delete-account="${a.id}" title="Delete">×</button>`;
    list.appendChild(row);
  });
  list.querySelectorAll('[data-rename-account]').forEach(btn=>{
    btn.addEventListener('click',()=>{
      const a=getAccount(btn.dataset.renameAccount); if(!a) return;
      const name=prompt(`Rename "${a.name}" to:`,a.name);
      if(name&&name.trim()){ a.name=name.trim(); save(); renderAccountsList(); renderAll(); }
    });
  });
  list.querySelectorAll('[data-delete-account]').forEach(btn=>{
    btn.addEventListener('click',()=>{
      const id=btn.dataset.deleteAccount; const a=getAccount(id); if(!a) return;
      if(!confirm(`Delete "${a.name}"? Entries will keep their history but lose this ${accountLabel().toLowerCase()} assignment.`)) return;
      state.accounts=state.accounts.filter(x=>x.id!==id);
      state.entries.forEach(e=>{ if(e.accountId===id) e.accountId=null; });
      state.tasks.forEach(t=>{ if(t.accountId===id) t.accountId=null; });
      if(state.activeTimer&&state.activeTimer.accountId===id) state.activeTimer.accountId=null;
      save(); renderAccountsList(); renderAll();
    });
  });
}
...
function saveEntry(){
  ...
  if(editingEntryId){
    ...
    state.entries.push({id:nextId('e'),projectId,subcategoryId,accountId,notes,taskId:null,startMs,endMs});
  }
...
function addAccount(){
  ...
  state.accounts.push({id:nextId('a'),name});
